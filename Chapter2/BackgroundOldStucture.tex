...
The more of the world that is the viewer's perception is moved towards only being able to sense the virtual world the more futher along the scale it becomes until a user is fully immesered in a virtual world. 
...
The Oculus \gls{vr} \gls{hmd} had recently been publicly released, which used a Fresnel lens~\cite{Cheng2022}. 
Meanwhile, the Meta lens \gls{ost} \gls{ar} \gls{hmd} utilized a distorted reflection of a screen's projection.
...
Participant comments indicated this was due to the difference in view windows between the Occlus and the Meta \glspl{hmd}.
...
This study used three different depth cues (points, lines, and boxes) as conditions and four different illumination ranges which to change the texure of the surface (Ambient, Half-Lamber, Blinn-Phong, Cook-Torrane). 
...
Some research has found that people tend to precive different colors as being closer or further away than others~\cite{Aoi2020, Ping2020a}.
...
Throughout this study, Aoi and Li~\cite{Aoi2020} noticed participants underestimated depth perception but it could be improved by using either (or both) binocular parallax and motiion parallax. 
...
A key focus of research has explored depth perception in \gls{X-ray Vision} as related to fixing the depth/distance offset caused by trying to place a virtual object on the other side of a real-world object in refernce to the user's view point~\cite{Furmanski2002, Ghasemi2018, Sandor2010}. This makes the work in this field done with depth perception different from other forms of research on depth perception. Some works are looking into methods to enhance the depth perception of visualization~\cite{Martin-Gomez2021, Fischer2023}. The major finding that seems prevalent in this field is that partial occlusion is an effective tool to improve depth perception.
...
This effect has not seen widespread use only being utalized by Kitajima et al.~\cite{Kitajima2015} who manually created a depth based AR system where the plans were moved so a user could only focus on one to detemine if they could regain tell if what was infront or behind.
...
\begin{figure}
    ...
    \caption[Wireframe, Random Dot, and Grid \gls{X-ray Visualization} over colorful patterned box with a virtual cube, sphere icosahedron, and a Stanford bunny rendered inside.]{This is a demonstration of real world overlay's and how they are utalized as forms of X-ray vision. the \glspl{X-ray Visualization} Wireframe, Random Dot, and Grid and are placed over colorful patterned box with a virtual cube, sphere icosahedron, and a Stanford bunny rendered inside. The image was taken using the HoloLens' screen view. }
...
This is separate from the Halo technique as these do not generally utalize the image of the real world while the halo technique utalizes the shape of the object that has been inserted. 
...
\begin{figure}[tb]
    ...
    \caption[A description of the \gls{X-ray Vision} system used by Kalkofen et al.~\cite{Kalkofen2013}]{A description of the \gls{X-ray Vision} system used by Kalkofen et al.~\cite{Kalkofen2013}. This image shows the various components of the graphics pipeline requried to create the adaptive ghosting effect.
    (a) The occluder and occluded elements are combined into a ghosted view using state-of-the-art transparency mapping based on the importance map of the occluder. However, this initial ghosted view lacks is difficult to intergrate into the real world seamlessly as it is either used to cover a area or it is not. To address this, a new importance map is generated for the resulting ghosted view which looks at area's of high contrast and places the visulisation on to them as well as over the rest of any are's close to virtual elements that need to be seen within the physical objects.
...
The application of X-ray vision focuses on specific domains where the visualization of internal structures and hidden information are important.  
X-ray vision technologies are prominently featured in the medical, construction, and maintenance domains. In addition, generic techniques may be applicable in multiple domains, as shown in the Generic category in \autoref{fig:XRayVisionUseCaseDistribution} and \autoref{fig:LitReviewDevicesBeingUsed}. Security and Navigation each presented several examples, while few examples of X-ray vision were encountered in the remaining application areas.

\begin{figure}[tb]
    \centering
    \includegraphics[width=\textwidth]{Chapter2/Images/XRayVisionUseCaseDistribution.png}
    \caption{A bar chart representing research aimed at a given application of \gls{X-ray Vision}.}
    \label{fig:XRayVisionUseCaseDistribution}
\end{figure}

All of the generic papers presented in \autoref{fig:XRayVisionUseCaseDistribution} and \autoref{fig:LitReviewDevicesBeingUsed} tend to present important information relating to fundamental aspects/tasks/challenges that can be applied to a wide range of situations. These include navigating through one’s home or office, interacting with robots, determining the distance between an object and a user, or simply proving how to place items in a location. The findings of these papers are relevant to many fields. These findings span a diverse range of novel research with the development of visualizations primarily tailored for these purposes, offering foundational guidance for domain-specific papers~\cite{Kalkofen2007, Kitajima2015, Otsuki2015, Sandor2010}.

\paragraph{Medicine:}
In the medical research domain, \gls{X-ray Vision} integration supports in-situ diagnostic capabilities and surgical activities. \gls{X-ray Vision} allows medical professionals to visualize virtual information related to internal structures, such as bones, organs, and tissues, in real-time overlays onto a patient's body during examinations or procedures~\cite{Bichlmeier2007, Blum2012, Erat2018, Habert2015, Lerotic2007}. \gls{X-ray Vision} can enhance diagnostic accuracy by providing an immediate, three-dimensional view of anatomical structures, aiding in identifying abnormalities or subtle variations~\cite{Bichlmeier2007, Blum2012}.

During surgeries, \gls{X-ray Vision} could potentially enable surgeons to navigate complex anatomies, guiding them to target specific areas and confidently perform invasive procedures. 
The works encountered in the literature present technologies validated by case studies demonstrating how such solutions could be adopted in diagnostic and interventional aspects of healthcare~\cite{Bajura1992}. 
However, further work is needed to mature the technology and align it with clinical requirements and regulations to enable the clinical use of such technologies on a routine basis~\cite{Pratt2018}. 

% Talking about Seilhorst's experiment
When asking surgeons for their expertise, Sielhorst et al.~\cite{Sielhorst2006} and Bichlmier et al. ~\cite{Bichlmeier2007} tried to visualize a set of data within the patient's body similar to Bajura et al.\cite{Bajura1992}. 
...
These seven conditions included three different types of rendering of a spine: a 3D iso-surface, the spine rendered using \gls{dvr}, and a wireframe model of the iso-surface, a transparent version of the iso-surface.
The four conditions utilized other forms of \gls{X-ray Vision} and changed their visibility using various \gls{X-ray Vision} techniques.
...
Another one of these utilized a virtual hole that sat over the iso-surface and \gls{dvr} version of the rendering.
The surgeons all noted that the sense of depth was not adequate, and the \gls{dvr} rendering was too difficult to use as it could not render in real-time, but they did note that it was more accurate to place the object here~\cite{Sielhorst2006, Bichlmeier2007}. 

\paragraph{Construction and Maintenance:}
In construction and maintenance, \gls{X-ray Vision} enables construction professionals to see through physical structures. This facilitates a comprehensive understanding of internal structure and integrity and identifies potential issues that may be otherwise hidden. Such capability can support building inspections, allowing real-time visualization of hidden structural elements, electrical wiring, and plumbing systems~\cite{Becher2021, Eren2018, Muthalif2022}. Moreover, it can help pinpoint areas requiring attention within existing structures. By providing a transparent view into the inner workings of buildings, \gls{X-ray Vision} enhances construction quality control, accelerates maintenance procedures, and contributes to the safety of built environments~\cite{Feiner1995, Liu2018}.

Construction and architecture have been a long-running goal of \gls{X-ray Vision}. Feiner et al.~\cite{Feiner1995} wrote one of the first papers looking into \gls{X-ray Vision}. It was focused on giving the user the impression of looking through walls by rendering them in \gls{ar} and gradually making them more occlusive as the user is looking through them.
...
Laying the visualization of underground pipes comes up in several papers~\cite{VanSon2018, Eren2018, Muthalif2022, Becher2021}, and the techniques for dealing with this are either overlaying schematics over the real world~\cite{VanSon2018} or rendering them underground using \gls{X-ray Vision}.
Eren et al.~\cite{Eren2013} originally proposed using \gls{X-ray Vision} with underground pipes for mobile \gls{ar} applications with a series of \gls{X-ray Vision} techniques shown in \autoref{fig:Eren2013}. 
Later, Eren et al.~\cite{Eren2018} conducted a study to measure how accurately Virtual \gls{X-ray Vision} could be judged. 
This study used a mobile device fixed at a height of 150 cm. The pipes were placed horizontally 2.5 meters from the participants, and different \glspl{X-ray Visualization} were used.
...
Muthalif et al.~\cite{Muthalif2022} explored over 6 different X-ray vision prototypes using 6 combinations of the examples. \gls{X-ray Vision} methods of looking through ground and walls using three different methods using a virtual box with some variants.
One of these variants had lines to help measure the depth into the ground the visualization was displaying. 
Another used interaction as a way of illustrating \gls{X-ray Vision} by using a slider to allow the user to control how deep the virtual box is. 
Their final method showcased caused shadows to appear against the walls of the virtual box, allowing for accurate depth perception even when items did not collide with the edges of the visualization. 

\paragraph{Security:}
In the security industry, \gls{X-ray Vision} can augment situational awareness and reveal potential security threats. \gls{X-ray Vision} has the potential to reveal persons or objects obscured by walls and other objects, support situational awareness by melding real-time surveillance information with the security personnel’s view of the immediate environment, and identify concealed items. Although use cases in the security domain may benefit from the ability afforded by \gls{X-ray Vision} to see through and around physical objects, relatively few user studies of such applications have been found in the literature. A possible explanation may be that such applications are commercially sensitive and, hence, not published in the academic literature~\cite{Kameda2004, Livingston2005, Ohta2010, Phillips2020, Tsuda2005}. 

Monitoring security cameras can be difficult, as covering large, dense areas requires a security officer to concentrate on multiple screens at once.
This prompted Kameda et al.~\cite{Kameda2004}, Tsuda et al.~\cite{Tsuda2005}, and Ohta et al.~\cite{Ohta2010} to create a system designed to look through buildings utilizing \gls{perspective_corrected_projection}.
The images are calibrated using location landmarks that can be found from both cameras~\cite{Kameda2004}. 
The \gls{X-ray Vision} was initially performed by making the foreground transparent. 
Later, several different \glspl{X-ray Visualization} were investigated using people who were experienced with the original security system.
Users of the system claimed that more occlusive \gls{X-ray Vision} effects were more useful. These occlusive X-ray vision did work the best for allowing the staff to be aware of the surrondings, but it was noted. 
This would have been most effective if the \gls{X-ray Vision} could have been toggled on and off as this would have allowed the users to have a clearer view of the environment when needed~\cite{Kameda2004, Tsuda2005, Ohta2010}.

\paragraph{Navigation and Tourism}
For spatial and location-based activities, \gls{X-ray Vision} has been proposed to support the user’s ability to see beyond their immediate physical environment, enabling users to see inside and through built structures. Such an extended view has been proposed to aid navigation tasks.  These include showing navigational cues on a road that can respond to depth cues (Yasuda and Ohama 2012), looking through buildings to understand what is on the other side to give an increased sense of space~\cite{Dey2011, Maia2016}, and even navigational experiences closer to “time travel” such as the sight of the historic appearance of modern cities in situ~\cite{Yamamoto2014}.

Yamamoto et al. ~\cite{Yamamoto2014} developed an \gls{ar} application that utilized \gls{X-ray Vision} to view underground ruins for mobile devices. They created a cubic environment map with a 3D model of the ruin and panorama photos, then implemented a movable viewpoint to simulate a 3D effect with alpha blending. This approach improves user perception by allowing the viewpoint to move according to the device’s orientation, providing a parallax effect and making the objects appear more three-dimensional.

As time moves forward, buildings rise and fall, and \gls{X-ray Vision} is capable of explaining to users the layout cities used while seeing the appearance of the old buildings.
Maia et al.~\cite{Maia2016} created a mobile \gls{ar} application that utilized \gls{X-ray Vision} to illustrate the buildings that used to be and the previous layout of the city.
This system was evaluated using a SUS questionnaire with positive responses.

Dey et al.~\cite{Dey2011} developed a mobile \gls{ar} \gls{X-ray Vision} system designed for pedestrian navigation and evaluated it by comparing it with two traditional navigation applications (North-up and View-up maps).
Participants walked a 900-meter route with three checkpoints using one of the navigation aids. The study found that the \gls{ar} \gls{X-ray Vision} system significantly reduced the number of context switches (shifts between looking at the phone and the environment) compared to traditional maps. 
This suggests that an \gls{ar} enabled \gls{X-ray Vision} system may serve to further enable navigation systems in the future. 
...
\paragraph{Education:}
Santos et al.\cite{Santos2015} have produced some work looking into the effect that \gls{X-ray Vision} has when educating school children but did not find any major findings regarding \gls{X-ray Vision} itself. Instead, they discovered that \gls{ar} did make students more excited about learning~\cite{Santos2015}. It is quite likely more work could be done, exploring avenues of using \gls{X-ray Vision} in anatomical classes as there has been evidence that this may improve retention of information regarding human anatomy~\cite{Akpan2019}. 

\paragraph{Video Games:}

\gls{X-ray Vision} research in games and entertainment appears to be rare, according to the results shown in \autoref{fig:XRayVisionUseCaseDistribution}~\cite{Guo2023}. 
%This is likely because there is a lot of overlap between these fields and generic topics.  
However, several augmented reality video games like Microsoft’s RoboRaid~\footnote{\url{https://www.microsoft.com/en-us/p/roboraid/9nblggh5fv3j?activetab=pivot:overviewtab}} and Dr Grordbort's Invaders~\footnote{\url{https://world.magicleap.com/en-us/details/com.magicleap.invaders}} (\autoref{fig:DrGrordbortsInvaders}) do exist in the space. Gaming strives to take \gls{X-ray Vision} into new territories and finds different methods of interacting with the virtual world. This lends itself to aiding usability, but the field still has a lot to learn about how \gls{X-ray Vision} can and should be used. 
...
\autoref{fig:DispalyModesUsedForXRayVis} shows there are preferences between different types of \gls{X-ray Vision} devices to an extent due to the individual benefits and disbenefits caused by these displays which have been laid out in \autoref{tab:XRayVisionDeviceComparison}.
...
Illustrative techniques can also include the scope of non-photorealistic rendering, like using cel-shading and the deformation of video footage to look like it was produced by a pencil or paintbrush~\cite{Lawonn2018}. 
...
To improve the on thier piror work~\cite{Interrante1995}, Interrante~\cite{Interrante1997a} created a visualization that utilized valleys, ridges, and curvature to explain the objects' flow. 
...
Lawonn et al.~\cite{Lawonn2017} then ran a study comparing their version of hatching to the same cylinder from thier previous study~\cite{Lawonn2017} with pseudo-chroma depth rendering and Phong shading.
...
This method of stippling was later extended by Ma et al.~\cite{Ma2018} to allow for the stippling to be applied to a 3D model in real time that utalized pre-computation to speed up the process rather than a more complex shader.
...
\end{itemize}
Since this thesis is focused on real-time volume rendering for immersive \gls{mr} devices, a new volume was required to be generated each frame. This section will have more of a focus on papers that utilize the Geometric Definition Method. Which has since been heaver extended by other to now utalize more modern \gls{gpu} techniques like fragment shaders.
...
This allowed the lighting to react to certain elements and allow different textures within the volume to have a different appearance even if the Hounsfueild unit was similar at that \gls{voxel_g}. 
...
Lu et al.\cite{Lu20} created a system that was able to apply stipping to a complete volume. 
...
Surfaces that were facing the camera were faded out, and the lighting algorithm mentioned in Lu et al.~\cite{Lu20} allowed for shading to become an option to be utilized by the final visulization.
...
This shape perception study showed that raycasted shadows were not essential for shape perception, but instead, the \gls{ui} was because partcipants were able to orientate their depth perception by with the abiltiy to rotate the object.
...
The first study was focused on testing if there was any noticeable benefit to \gls{mr} when needing to utilize a \gls{cave} using the resticted motion tracking~\cite{Laha2012}. 
...
An early version of volumetric hatching used in this diseration was inspired by research from Feng et al.~\cite{Feng20152548}, which helped establish fundamental approaches to illustrative rendering.
...
\begin{figure}
    ...
    \caption[
        ...
        {
            The 6 datasets used in Grosset et al.'s~\cite{Grosset2013} research: (a) aneurism, (b) backpack, (c) bonsai, (d) flame, (e) Richtmyer-Meshkov instability \& (f) thorax. Used with permission from IEEE \textcopyright{} 2013.
        }
    \label{fig:Grosset}
\end{figure}
...
~\footnote{The descriptions used in Englund et al.'s~\cite{Englund2016, Englund2018} may be missleading when compared to other research. So the names they have used are in parenthesis.} : Depth Darkening (Depth Darkening),
...
Graphical capabilities on \gls{mr} \glspl{hmd} require more simultaneous processes than on traditional displays as they as they require the rendering of multiple dispalys which need to be redrawn from a slightly different angle each frame, elimiating current hardware accelleration approaches~\cite{Geng2013}.
...
So, in a bid to allow our datasets to be utilized in HCI studies, this type of visualization allowed us to perform tasks that could be repeated in a numerious (over a septillion) different ways and modified for many different studies.  
