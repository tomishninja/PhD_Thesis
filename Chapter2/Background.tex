...
The ‘contextual’ aspect reflects how the cutout is positioned and adjusted based on the user’s interaction or viewpoint, while the ‘anatomic’ component identifies its focus on anatomical layers rather than general geometry. 
‘Mimesis’ highlights that the effect intentionally imitates the look and structure of anatomical illustration rather than attempting a physically accurate X-ray. 
Together, the term describes the specific way these familiar rendering methods are combined for anatomical depiction.
...
They had surgical students utilize this system with laparoscopic video to evaluate it. 
The original authors described the technique as being “positively received” by participants; however, their study focused exclusively on collecting subjective impressions. 
No quantitative assessments of depth perception, task performance, or usability were reported which is common in early stage desing applications where authors may prioritize a conceptual demonstration over a rigorous evaluation.
As a result, the evidence available is limited to subjective feedback rather than objective performance metrics, and this thesis reflects that scope accurately.~\cite{Johnson2014}. 
...
This effect goes by many names but is most commonly referred to as Halo or Silhouette, as it produces an outline around virtual objects. For simplicity, this dissertation will use the term Halo visualization.
The Halo \glspl{X-ray Visualization}, as described by Ozgur et al.~\cite{Ozgur2017}, presents the effect as a bright rim around the edges of the object of interest and uses alpha blending to convey depth~\cite{Ozgur2017}.
This effect closely resembles what is known as the “silhouette” technique in medical visualization~\cite{Rheingans2001}. Strictly speaking, “silhouette” refers to halo-like outlines that exclude regions around intersections, but terminology varies across fields. 
Therefore, for clarity and consistency, this dissertation uses Halo visualization to refer to this class of effects
\autoref{fig:JustHalo} illustrates how the halo is configured to leverage the depth-perception cue of relative size. This \gls{X-ray Vision} effect is distinctive in that it does not introduce occlusion, making it well suited to surgical scenarios~\cite{Ozgur2017}. 
It has been evaluated on a monoscopic display in a study designed for minimally invasive surgery, and it can also incorporate color-based cues that use visual language to further communicate depth to the user~\cite{Ozgur2017}.
...
Another set of studies published by Martin-Gomez et al.~\cite{MartinGomez2021} studied the difference between four \glspl{X-ray Visualization} (Virtual hole, ghosting, and Random Dot) along with a base line condition that superimposed the data. This reserach utalized both \gls{vst} \gls{ar} and \gls{ost} \gls{ar} devices in the near field. 
